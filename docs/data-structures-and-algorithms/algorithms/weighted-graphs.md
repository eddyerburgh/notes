...
Checking that vertices are in the same components can be done in $$O(\log n)$$ by using a [union-find]({{ '/data-structures-and-algorithms/data-structures/union-find' | relative_url }}) data structure. This means the running time depends on the sort. If the sort is $$O(n\log n)$$ then algorithm is $$O(m\lg m)$$ (where $$m$$ is the number of edges) {% cite algorithm-design-manual -l 197 %}.
